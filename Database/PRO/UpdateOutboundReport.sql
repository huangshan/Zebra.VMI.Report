...
BEGIN
	...
	SELECT ID, SHIPTITLE, STATUS, ISNULL(SLFLAG, 0), ISNULL(NEWORDER,'Y'), ISNULL(ISPACKED, 'N'), ISNULL(ISCZ, 'N'), ISNULL(ISPAYMENT, 'N'), ISNULL(ISOUTBOUND, 'N'), ISNULL(REFVALUE, ''), ISNULL(PARSESTATUS, ''), ISNULL(SENDBACKFLAG, 0), SUCODE FROM VMI_OUTBOUNDREPORT WITH(NOLOCK) WHERE SUCODE NOT IN ('CSC44453', 'CSC40467') AND STATUS<>'C' AND ISNULL(SLFLAG, 0)<>4 UNION ALL
	SELECT ID, SHIPTITLE, STATUS, ISNULL(SLFLAG, 0), ISNULL(NEWORDER,'Y'), ISNULL(ISPACKED, 'N'), ISNULL(ISCZ, 'N'), ISNULL(ISPAYMENT, 'N'), ISNULL(ISOUTBOUND, 'N'), ISNULL(REFVALUE, ''), ISNULL(PARSESTATUS, ''), ISNULL(SENDBACKFLAG, 0), SUCODE FROM VMI_OUTBOUNDREPORT WITH(NOLOCK) WHERE SUCODE ='CSC44453' AND PARSESTATUS<>'N' AND STATUS<>'C' AND SENDBACKFLAG=0 UNION ALL
	SELECT ID, SHIPTITLE, STATUS, ISNULL(SLFLAG, 0), ISNULL(NEWORDER,'Y'), ISNULL(ISPACKED, 'N'), ISNULL(ISCZ, 'N'), ISNULL(ISPAYMENT, 'N'), ISNULL(ISOUTBOUND, 'N'), ISNULL(REFVALUE, ''), ISNULL(PARSESTATUS, ''), ISNULL(SENDBACKFLAG, 0), SUCODE FROM VMI_OUTBOUNDREPORT WITH(NOLOCK) WHERE SUCODE ='CSC40467' AND STATUS<>'C' AND SENDBACKFLAG=0
	OPEN REPORT_CURSOR
	FETCH FROM REPORT_CURSOR INTO @ID, @SHIPTITLE, @STATUS, @SLFLAG, @NEWORDER, @ISPACKED, @ISCZ, @ISPAYMENT, @ISOUTBOUND, @REFVALUE, @PARSESTATUS, @SENDBACKFLAG, @SUCODE;
	WHILE @@FETCH_STATUS=0
	BEGIN
	 IF @SUCODE ='CSC44453' AND @PARSESTATUS='P'
		BEGIN
			SELECT @PARSESTATUS=C.STATUS FROM API_BASRECEIVEJOB A
			LEFT JOIN VMI_MESSAGEPARSERECORD C ON(A.JOBID=C.JOBID)
			WHERE A.REF_VALUE=@REFVALUE;
		END
	 IF @SUCODE ='CSC44453' AND @PARSESTATUS='P'
		BEGIN
			GOTO LABLE_FETCH
		END
	 IF @SUCODE IN ('CSC44453', 'CSC40467')
		BEGIN
			SELECT @SLCODE=SL.SLCODE, @SHIPTITLE=SL.SHIPTITLE, @SLAREA=SL.SLAREA, @STATUS=SL.STATUS, @INTODATE=SL.INTODATE
			FROM OSSHIPLIST_PRE SL WITH(NOLOCK)
			WHERE TRANSNUM=@REFVALUE;
		END
	 IF @STATUS <> '1'
		BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					END
				END
				IF @ISOUTBOUND = 'Y' AND @SUCODE IN ('CSC44453', 'CSC40467')--出库回传
				BEGIN
					SELECT
					@SENDBACKFLAG = ( CASE WHEN ISNULL(GELS_SENDFLAG,'')='0' THEN 1
									 WHEN  ISNULL(GELS_SENDFLAG,'')='1' AND  ISNULL(FAILCOUNT,0)<10 THEN 0
									 WHEN  ISNULL(GELS_SENDFLAG,'')='1' AND  ISNULL(FAILCOUNT,0)>=10 THEN 2
									END)
					FROM API_STOCKOUT WITH(NOLOCK) WHERE SHIPORDER_ID=@SHIPTITLE AND  STATUS='2' AND (SEND_TYPE=@SUCODE) AND ISNULL(FAILCOUNT,0)<10;
				END
			END
		END
...
DOUPDATE:
	...
	,PARSESTATUS=@PARSESTATUS, SENDBACKFLAG=@SENDBACKFLAG
	WHERE ID=@ID;
