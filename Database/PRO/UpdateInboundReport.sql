...
BEGIN
	...
	SELECT ID, REF_VALUE, PARTNER, PARSESTATUS, isnull(PostFlux,0), SEFLAG FROM VMI_INBOUNDREPORT WHERE ISNULL(LOTNO,'')<>'' AND (SEFLAG=0 OR SEFLAG=1) AND PARTNER ='CSC40467'
	OPEN REPORT_CURSOR
	 ...
	 BEGIN
		 ...
		 IF @PARSESTATUS = 'Y'
			BEGIN
				...
				UPDATE VMI_InboundReport SET PARSESTATUS=@PARSESTATUS, POSTFLUX=@POSTFLUX, SEFLAG=@SEFLAG
				WHERE ID=@ID;
			END
	END
	IF @PARTNER <> 'CSC44453'
